...
export default function Index({ filter, setfilter, t, loading, Data, darkMode, DataType, setDataType }) {
	const { pathname } = useLocation();
	const Language = Cookies.get("i18next") || "en";

	const ExpandedComponent = ({ data }) => <ExpandedComp data={data} darkMode={darkMode} Language={Language} />
	createTheme(
		"solarized",
		{
			text: {
			...
			<Card.Body>
				<Row>
					...
					>
						{!loading ? (
							<DataTable
								title={pathname.includes('collocations') ? `${t('Collocations')}` : `${t('Phrasal verbs')}`}
								columns={columns}
								data={Data.filter(ele => ele?.en?.Name.toString().startsWith(`${DataType}`))}
								highlightOnHover
